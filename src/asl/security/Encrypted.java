...
public class Encrypted {
	...
	// constructor(s)
	public Encrypted(byte[] key) throws NegativeArraySizeException,
			NullPointerException {
		this.key = Arrays.copyOf(key, key.length);
	}

	public Encrypted(byte[] iv, byte[] cipherText, byte[] hmac)
	...
	private boolean readyForDecrypt() {
		return (key == null) ? false : (iv == null) ? false
				: (hmac == null) ? false : (cipherText == null) ? false : true;
	}

	// cryptographic routines
	protected boolean encrypt(String plainText) throws EncryptionException {
		...
		if (readyForEncrypt()) {
			try {
				...
				// generate the HMAC
				hmac = Mac.getInstance(HMAC_ALGORITHM).doFinal(
						plainText.getBytes(CHAR_ENCODING));
	...
	protected String decrypt() throws EncryptionException {
		...
		if (readyForDecrypt()) {
			try {
				...
				if (Arrays.equals(hmac, Mac.getInstance(HMAC_ALGORITHM)
						.doFinal(clearText))) {
