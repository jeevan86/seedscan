 ...
 * and an array of 32-bit "in-the-clear" ints with a chunk of time series and
 * this routine will output mini-seed blockettes ready for writing to the data
 * system. (In fact they are written using "IndexBlock.writeMiniSeed()"). This
 * keeps a static list (via TreeMap) of the channels known to this routine which
 * it then uses to keep the context of the mini-seed data going.
 * 
 * Notes on Steim2 compression : The Steim 2 contains 0-N data differences the
 * value of data[0] and data[n-1]. The first difference always takes data[0] to
 * the last value of the previous frame. So, the only time you need outside
 * information is for the first buffer if you are trying to get the first
 * difference to match. If not, the first difference can be zero and the prior
 * reverse constant is unknowable. To support making this first difference be
 * right during testing, the ability to pass the last value is supported in the
 * call to add timeseries. Practically there is no reason to pass such a value
 * other than for trying to exactly match some known series.
 * 
 * @author davidketchum
 */
...
public class RawToMiniSeed {
	...
	static private final Map<String, RawToMiniSeed> chans = (Map<String, RawToMiniSeed>) Collections
			.synchronizedMap(new TreeMap<String, RawToMiniSeed>()); // The list
																	// of known
																	// channels
	static private final Map<String, RawToMiniSeed> secondChans = (Map<String, RawToMiniSeed>) Collections
			.synchronizedMap(new TreeMap<String, RawToMiniSeed>()); // The list
																	// of known
																	// channels
	static private final Map<String, RawToMiniSeed> oob = (Map<String, RawToMiniSeed>) Collections
			.synchronizedMap(new TreeMap<String, RawToMiniSeed>()); // The list
																	// of known
																	// channels
	...
	private String stringFromUSec(long usec) {
		...
		int sec = ms / 1000;
		ms = ms % 1000;
		return "" + df6.format(ymd[0]).substring(2, 6) + "-"
				+ df6.format(ymd[1]).substring(4, 6) + "-"
				+ df6.format(ymd[2]).substring(4, 6) + " "
				+ df6.format(hr).substring(4, 6) + ":"
	 ...
	 * 
	 * @return the absolute microseconds of last data sample in this compression
	 *         rec
	 */
	public long getJulianUSec() {
		return (long) (((long) julian * 86400000000L) + difftime + ndata
				* periodUSec);
	}
	 ...
	 * 
	 * @param obj
	 *            An object which implements MiniSeedOutputHander to use for
	 *            putbuf()
	 */
	 ...
	 * 
	 * @param name
	 *            The seedname of the channel to create
	 * @param rt
	 *            nominal data rate of the channel
	 * @param nFrames
	 *            Number of frames to put in the output Mini-seed (7 for 512, 63
	 *            for 4096)
	 * @param year
	 *            Year of the first sample for this
	 * @param doy
	 *            Day-of-year of the first sample
	 * @param sec
	 *            integer number of seconds since midnight for 1st sample
	 * @param micros
	...
	public RawToMiniSeed(String name, double rt, int nFrames, int year,
		...
		hdr = new FixedHeader(seedname, rt, numFrames, year, doy, sec, micros,
				startSequence);
		difftime = sec * 1000000L + micros; // no data in time buffer, set it to
											// current
		earliestTime = difftime;
		// This is a runtime exception catch for impossible Julian day
		// conversion to try to
		...
		try {
			...
			julian = SeedUtil.toJulian(1972, 1); // give it some Julian day just
													// in case
		}
		ndata = 0;
		rate = rt;
	...
	/**
	 * calculate the gap from the last sample in this RawToMiniSeed stream and
	 * the time represented by sec and micros
	 * 
	 * @param year
	 *            The year
	 * @param doy
	 *            The day of year
	 * @param sec
	 *            Time since midnight of first sample
	 * @param micros
	...
	/**
	 * calculate the gap from the first sample in this RawToMiniSeed stream and
	 * the time represented by sec and micros
	 * 
	 * @param year
	 *            The year
	...
	public long gapCalcBegin(int year, int doy, int sec, int micros, int n) {
		...
		// if the gap is on the order of days, check to make sure it is not a
		// day problem
		/*
		 * if(Math.abs(gap) >= 86399999999L) { int jul =
		 * SeedUtil.toJulian(year,doy); if(jul != julian) { gap = gap + (jul -
		 * julian)*86400000000L; } }
		 */
		return gap;
	}
	...
	public synchronized void process(int[] ts, int nsamp, int year, int doy,
		...
		// iIf all of these are true a force out has occurred, this is new data,
		// do not check for gaps
		if (currentWord == 0 && reverse == 2147000000 && ns == 0
				&& currentFrame == 0) {
			hdr.setStartTime(year, doy, sec, micros);
			earliestTime = sec * 1000000L + micros;
		}
		// If this is earlier than this first, check for continuity the other
		// way!
		else {
			if ((sec * 1000000L + micros) < earliestTime) {
				gap = gapCalcBegin(year, doy, sec, micros, nsamp);
				if (dbg)
					sb.append("RTMS: " + seedname + " " + earliestTime()
				...
				hdr.setStartTime(year, doy, sec, micros);
				// else
				// prt("RTMS: "+seedname+" "+earliestTime()+" add begin continuous");
				earliestTime = sec * 1000000L + micros;
				difftime = earliestTime;
			...
			// Its later than first time, see if it is a gap at the end
				...
				// If this gap is bigger than 1/2 sample (1/2 sample is
				// 500000./rate in micros)
				if (Math.abs(gap) > 500000. / rate) {
					if (dbg)
						sb.append("RTMS: " + seedname + " " + lastTime()
					...
					if (dbg)
						prta("RTMS: " + seedname + " " + lastTime()
								...
								+ " rev=" + reverse + " lst=" + lastValue);
					forceOut(reverse);
					clear();
					earliestTime = sec * 1000000L + micros; // set initial time
															// to this buffer
					difftime = earliestTime;
					hdr.setStartTime(year, doy, sec, micros);
				}
		...
		// Set the flags for all putbufs from this call to process according to
		// the flags
		...
		int[] x = new int[Math.max(0, ndata) + nsamp]; // space for the time
														// series
		...
		if (reverse == 2147000000) {
			d[0] = x[0] - lastValue; // starting up you do not know the reverse
		} else {
			d[0] = x[0] - reverse; // else use it to get first difference
			if (lastValue != reverse && dbg)
				sb.append(seedname + " Last != reverse " + lastValue + " != "
						+ reverse + "\n");
		}
		...
		int dnib = -1; // per manual coding of 2nd key (stored in data word high
						// order)
		...
		while (somePacked) {
			...
			if ((somePacked = packOK(d, nextDiff, 7, 8))) { // 7x4 bit diffs
				...
				npack = 7;
				bits = 4;
			} else if ((somePacked = packOK(d, nextDiff, 6, 16))) { // 6x5 bit
																	// diffs
				ck = 3;
				dnib = 1;
				npack = 6;
				bits = 5;
			} else if ((somePacked = packOK(d, nextDiff, 5, 32))) { // 5x6 bit
																	// diffs
				ck = 3;
				dnib = 0;
				npack = 5;
				bits = 6;
			} else if ((somePacked = packOK(d, nextDiff, 4, 128))) { // 4x8 bit
																		// diffs
				ck = 1;
				dnib = -1;
				npack = 4;
				bits = 8;
			} else if ((somePacked = packOK(d, nextDiff, 3, 512))) { // 3x10 bit
																		// diffs
				ck = 2;
				dnib = 3;
				npack = 3;
				bits = 10;
			} else if ((somePacked = packOK(d, nextDiff, 2, 16384))) { // 2x15
																		// bit
																		// diffs
				ck = 2;
				dnib = 2;
			...
			if (somePacked) {
				// If this packing crosses the day boundary, adjust it so it
				// does not and cut off
				// midnightProcess insures this is only dones once in a process
				// buffer
				long begbuf = ((long) sec * 1000000L + micros + (nextDiff
						+ npack - ndata)
						* periodUSec);
				// if(dbg)
				// prta("RTMS: "+seedname+" test midnight "+begbuf+" mid="+midnightProcess);
				if (!midnightProcess
						&& (begbuf > (86400000000L + periodUSec - 1L) || // This
																			// is
																			// on
																			// the
																			// next
																			// day
						begbuf <= 0)) { // packet on new day, ndata on other
										// side
					...
					prta("RTMS: " + seedname + " EOD cut off ndata begbuf="
							...
							+ micros + " period=" + periodUSec + " data.len="
							+ data.length);
					if (nleft <= (nsamp + ndata - nextDiff)) { // Do we have
																// enough data
																// for a
																// midnight
																// cutoff

						// There are ndata that need to be forced into this
						// frame, move the desired samples
						// to the begining of the data buffer to fake forceOut()
						// to use these samples.
						for (int i = 0; i < nleft; i++)
							data[i] = x[nextDiff + i];
						ndata = nleft;
						frames[currentFrame].put(key, 0); // save any mods made
															// to key so
															// forceOut gets it
						if (nextDiff > 0)
							forceOut(x[nextDiff - 1]); // used faked data to
														// force it out
						else
							forceOut(reverse); // note : no call to clear() as
												// we are just faking
						...
						doy = newdoy;
						sec = sec - 86400;
						julian = julian2; // Do not force out again on day
											// change
						// hdr.setStartTime(ymd[0], doy, sec,
						// micros+(nextDiff-ndataold+ndata)*periodUSec);
						// earliestTime = sec*1000000L+micros;
						nextDiff += nleft; // adjust nextDiff for the number of
											// samps just out
						hdr.setStartTime(ymd[0], doy, sec, micros
						...
						// (long) ((nextDiff-ndata)/rate*1000000.);
						currentWord = 1;
						key = 0;
				...
				// Check to see if there is enough data remaining to do this
				// compression
				if (npack > (nsamp + ndata - nextDiff)) {
					for (int i = nextDiff; i < nsamp + ndata; i++)
						data[i - nextDiff] = x[i];
					frames[currentFrame].put(key, 0); // save the key for next
														// time

					// The time of the first data sample in the difference array
					// (for gap detection)
					// time of ndata + samples into the new data
					// (nextDiff-ndata) at rate
					difftime = (long) sec * 1000000L + (long) micros
							+ (long) ((nextDiff - ndata) * periodUSec);
					julian = SeedUtil.toJulian(year, doy);
					if (difftime > 86400000000L) {
						// julian++; // Mar 5, 2008 - if data is compressed that
						// already does not cross the boundary, we commented
						// this
						// so the check at the top of the routine would cause
						// the data to be cut off on julian day skip.
						difftime -= 86400000000L;
					}
					ndata = nsamp + ndata - nextDiff;
					// reverse is the data value just proceeding the difference
					// array
					if (nextDiff > 0)
						...
						reverse = data[0] - d[0]; // 1/08 cut the BS - reverse
													// has to be data minus the
													// difference (to get same
													// diff!)
					}
					// else reverse=lastValue; // if we dont have the value, use
					// the last one!
				...
				key = (key << 2) | ck;
				if (dbg) {
					String s = "Word " + currentWord + " is " + ck + " dnib="
							+ dnib + " ns=" + ns + " diff(" + bits + " bit)=";
					for (int i = 0; i < npack; i++)
						s = s + d[nextDiff + i - npack] + " ";
					sb.append(s + "\n");
				}
				frames[currentFrame].put(w, currentWord);

				// Advance to next word, if this is end-of-frame, do the book
				// keeping
				currentWord++;
				if (currentWord >= 16) {
					frames[currentFrame].put(key, 0);
					if (dbg)
						sb.append("key=" + Integer.toHexString(key) + "\n");
					key = 0; // clear key for next run
					if (currentFrame == numFrames - 1) {
						if (dbg)
							sb.append("Word 2 of zero is hdr "
									+ Integer.toHexString(x[nextDiff - 1])
									+ "\n");
						frames[0].put(x[nextDiff - 1], 2);
						reverse = x[nextDiff - 1];
					}
					currentFrame++;
					currentWord = 1; // point to first data word, word 0 will
										// get the key

					// Is this new frame beyond the last frame (record is full?)
					if (currentFrame == numFrames) { // yes, output the record
														// and do book keeping
						currentFrame = 0;
						byte[] buf = new byte[64 + numFrames * 64];
						ByteBuffer bf = ByteBuffer.wrap(buf);
						bf.clear();
						hdr.setNSamp(ns); // set the number of samples in fixed
											// hdr
						...
						if (dbg)
							sb.append("Call putbuf with len=" + (numFrames + 1)
									* 64 + " override=" + overrideOutput + "\n");
						putbuf(bf.array(), (numFrames + 1) * 64);
						/*
						 * try { byte [] fr = new byte[numFrames*64];
						 * System.arraycopy(bf.array(), 64, fr, 0,
						 * numFrames*64); int [] samples = Steim2.decode(fr,
						 * hdr.getNSamp(), false);
						 * prt("NSamples returned="+samples.length); }
						 * catch(SteimException e)
						 * {prt("Put buf Steim Exception");}
						 */
						// calculate based on latest time available the start
						// time of next record
						hdr.setStartTime(
								year,
								doy,
	...
	/**
	 * this routine puts a complete mini-seed blockette in buf out to the
	 * correct file using the EdgeFile related routines. For debug purposes it
	 * stores the "newPutBuf" to indicate this was called by internal processing
	 * since the last time that flag was reset and puts a copy of the last
	 * written buffer in lastbuf.
	 */
	 ...
	 * of the counters. This is because day roll overs just fake up a
	 * data[ndata] and call this routine but do not want other data about the
	...
	public synchronized void forceOut(int reverse) {
		...
		if (ndata > 0) {
			int dnib = -1; // per manual coding of 2nd key (stored in data word
							// high order)
			int bits = 0; // number of bits in the packing
			int npack = 0; // Number of samples to pack
			int ck = 0; // coding of key in main key word
			...
			key = (key << 2) | ck;
			if (dbg) {
				String s = "Force out Word " + currentWord + " is " + ck
						+ " dnib=" + dnib + " ns=" + ns + " diff(" + bits
						+ " bit)=";
				for (int i = 0; i < npack; i++)
					s = s + d[nd + i - npack] + " ";
				sb.append(s + "\n");
		...
		while (currentWord < 16) {
			...
			frames[currentFrame].put(0, currentWord); // zero out the unused
														// words in the frame
			currentWord++;
		}
		frames[currentFrame].put(key, 0);
		...
		if (ndata <= 0)
			...
			frames[0].put(data[ndata - 1], 2); // reverse integration is last
												// data sample
		...
		// calculate based on latest time available the start time of next
		// record
	...
	/**
	 * return true of the given data differeces will bit in to ns samples of
	 * size limite by limit
	 */
	private boolean packOK(int[] d, int i, int ns, int limit) {
		...
		for (int j = i; j < i + ns; j++) {
			if (j < d.length) {
				if (d[j] < low || d[j] >= limit) { // <= is not needed < should
													// be o.k.
					ok = false;
					break;
				}
	...
	class FixedHeader {
		 ...
		 *            The Seed name
		 * @param rt
		 *            Nominal data rate in Hz
		 * @param nFrames
		 *            Number of steim 64 byte frames in the attached data
		 *            portion
		 * @param yr
		 *            Year of first sample in this Mini-seed blockette
		 * @param dy
		 *            Day-of-year of 1st sample
		 * @param sec
		 *            integral # of seconds since midnight
		 * @param micros
		...
		/**
		 * return the bytes of this Fixed header in a 64 byte array. Since a
		 * ByteBuffer is used to put data into the buffer, the data are in BIG
		 * ENDIAN order.
		 * 
		...
		public byte[] getBytes() {
			...
			buf.put((seedname.substring(2, 7) + seedname.substring(10, 12)
					+ seedname.substring(7, 10) + seedname.substring(0, 2))
					.getBytes());
			...
			buf.put(activity).put(IOClock).put(quality).put((byte) 2);// one
																		// block
																		// follows
			buf.putInt(0);
			buf.putShort((short) 64).putShort((short) 48);

			// This is the blockette 1000 (next block) encoding Steim2==11, Word
			// order 1 (big endian)
			buf.putShort((short) 1000).putShort((short) 56).put((byte) 11)
					.put((byte) 1);
			if (numFrames == 7)
				buf.put((byte) 9);
			...
			buf.putShort((short) 1001).putShort((short) 0)
					.put((byte) timingQuality).put((byte) usecs);
		 ...
		 * 
		 * @param n
		 *            The number of samples to which will be represented by this
		 *            header
		 */
		...
		public void setRate(double rate) {
			...
			if (rate < 1) {
				...
				if (Math.abs(r - Math.round(r)) < 0.0001) { // is it an even
															// period in seconds
					rateFactor = (short) (-r - .001); // yes, set it neg for
														// period mult 1
					rateMultiplier = (short) 1;
				} else {
					rateMultiplier = -1; // scale period to fit short, set mult
											// to be divisor
					while (r < 3275) {
						r = r * 10;
			...
			// Its > 1, so do hz
				...
				// check to see if this is an integer, if so, the GSN prefers
				// multipliers of 1
		 ...
		 * 
		 * @param a
		 *            The activity flags per the SEED documentation
		 */
		 ...
		 * 
		 * @param a
		 *            The Quality flags as per the SEED documentation
		 */
		 ...
		 * 
		 * @param a
		 *            The I/O and clock flags per the SEED docs
		 */
	...
	/**
	 * This class makes it easier to represent Steim Data frames of 64 bytes
	 * each for implementing the compressor. It basically allows putting and
