...
public class PlotMaker {
	...
	public PlotMaker(Station station, Channel channelX, Channel channelY,
			Calendar date) {
		this.station = station;
	...
	public void plotZNE_3x3(ArrayList<double[]> channelData, double[] xsecs,
			int nstart, int nend, String eventString, String plotString) {
		...
		final String plotTitle = String.format(
				"%04d%03d [Stn:%s] [Event:%s] %s", date.get(Calendar.YEAR),
				date.get(Calendar.DAY_OF_YEAR), station, eventString,
				plotString);
		final String pngName = String.format("%s/%s.%s.%s.png", outputDir,
				eventString, station, plotString);
		...
		if (!checkFileOut(outputFile)) {
			System.out
					.format("== plotZNE_3x3: request to output plot=[%s] date=[%s] but we are unable to create it "
							+ " --> skip plot\n", pngName, datestr);
			return;
		}
		...
		if (channelData.size() != channels.length) {
			...
			String message = String
					.format("== plotZNE_3x3: Error: We have [%d channels] but [%d channelData] for date=[%s]\n",
		...
		// Use the first data array, within the plotted range (nstart - nend) to
		// scale the plots:
		...
		// final XYPlot xyplot1 = new XYPlot((XYDataset)seriesCollection1, null,
		// verticalAxis, renderer);
		// final XYPlot xyplot1 = new XYPlot((XYDataset)seriesCollection1,
		// horizontalAxis, verticalAxis, renderer);
		final XYPlot xyplot1 = new XYPlot((XYDataset) seriesCollection1, hAxis,
				verticalAxis, renderer);
		...
		final XYPlot xyplot3 = new XYPlot((XYDataset) seriesCollection3, null,
				verticalAxisE, renderer);
	...
	public void plotPSD(double per[], double[] model, double[] psd,
			String modelName, String plotString) {
		plotPSD(per, model, null, null, psd, modelName, plotString);
	}

	public void plotPSD(double per[], double[] model, double[] nhnmPer,
			double[] nhnm, double[] psd, String modelName, String plotString) {
		...
		if (!checkFileOut(outputFile)) {
			...
			// + " --> skip plot\n", pngName );
			logger.warn(
					"== plotPSD: request to output plot=[%s] date=[%s] but we are unable to create it "
							+ " --> skip plot\n", pngName, datestr);
			return;
		}
		...
		final XYPlot xyplot = new XYPlot((XYDataset) seriesCollection,
				horizontalAxis, rangeAxis1, renderer);
	...
	public void plotCoherence(double per[], double[] gamma, String plotString) {
		...
		final String pngName = String.format("%s/%04d%03d.%s.%s-%s.%s.png",
				outputDir, date.get(Calendar.YEAR),
		...
		if (!checkFileOut(outputFile)) {
			...
			// + " --> skip plot\n", pngName );
			logger.warn(
					"== plotCoherence: request to output plot=[%s] date=[%s] but we are unable to create it "
							+ " --> skip plot\n", pngName, datestr);
			return;
		}
		...
		final XYPlot xyplot = new XYPlot((XYDataset) seriesCollection,
				horizontalAxis, rangeAxis1, renderer1);
