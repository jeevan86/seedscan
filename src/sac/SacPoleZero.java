...
public class SacPoleZero {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(sac.SacPoleZero.class);
	...
	public String toString() {
		...
		for (int i = 0; i < zeros.length; i++) {
			out += formatter.format(zeros[i].getReal()) + " "
					+ formatter.format(zeros[i].getImaginary()) + "\n";
		}
		out += POLES + " " + poles.length + "\n";
		for (int i = 0; i < poles.length; i++) {
			out += formatter.format(poles[i].getReal()) + " "
					+ formatter.format(poles[i].getImaginary()) + "\n";
		}
	...
	static Complex parseCmplx(String line) throws IOException {
		...
		String[] sline = line.split(" ");
		return new Complex(Float.parseFloat(sline[0]),
				Float.parseFloat(sline[1]));
	}
			...
			if (spz.poles.length != poles.length
				...
				for (int i = 0; i < poles.length; i++) {
					if (!closeFourDigit(spz.poles[i], poles[i])) {
						logger.error("pole " + i + " not equal"
								+ spz.poles[i].getImaginary() + " "
								+ poles[i].getImaginary() + " "
								+ spz.poles[i].getReal() + " "
								+ poles[i].getReal());
						return false;
					}
	...
	private static boolean closeFourDigit(double a, double b) {
		if (Math.abs(a - b) > 0.0001) {
			logger.error("fail closeFourDigit " + a + " " + b + " "
	...
	protected static DecimalFormat formatter = new DecimalFormat(
			" 0.0000;-0.0000");
	protected static DecimalFormat constantFormatter = new DecimalFormat(
			"0.0#######E00");
}
