...
public class EventComparePWaveOrientation extends Metric {
    ...
    /**
     * https://github.com/usgs/seedscan.git
    ...
    static void normalize(double[] northData, double[] eastData) {
        ...
        if (scaleFactor == 0) return; // avoid div by 0 error - true only if all inputs are 0
        for (int i = 0; i < northData.length; ++i) {
    ...
    static void normalize(double[] data) {
        ...
        if (max == 0) return; // don't change data if everything is 0-valued (should not happen)
