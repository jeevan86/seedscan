...
/**
 * @author crotwell Created on Aug 3, 2005
 */
public class CmplxArray2D {
	private static final Logger logger = LoggerFactory
			.getLogger(freq.CmplxArray2D.class);

	public CmplxArray2D(int x, int y) throws IllegalArgumentException {
	...
	public CmplxArray2D(float[][] real, float[][] imag)
		...
		if (real.length != imag.length) {
			throw new IllegalArgumentException(
					"real and imag arrays must have same length: "
							+ real.length + " " + imag.length);
		}
		for (int i = 0; i < real.length; i++) {
			if (real[0].length != real[i].length) {
				throw new IllegalArgumentException(
						"real array must be square: " + i + "  "
								+ real[0].length + " " + real[i].length);
			}
			if (real[i].length != imag[i].length) {
				throw new IllegalArgumentException(
						"real[i] and imag[i] arrays must have same length: "
								+ i + "  " + real[i].length + " "
								+ imag[i].length);
			}
		}
	...
	public final float mag(int x, int y) {
		return (float) Math.sqrt(getReal(x, y) * getReal(x, y) + getImag(x, y)
				* getImag(x, y));
	}
